...
;;; Moren environment DOM structure
;;;
;;; NOTHING
;;;
;;;
...
;;;(defparameter *jq-history-maxlen* 100)
...
(defun %repl-eval (input)
    (%js-try
     (handler-case
       ...
       (error (errmsg)
           (format *mordev-standard-error* "ERROR: ~a~%" (jscl::!condition-args errmsg))))
     (catch (err)
         (format *mordev-standard-error* "ERROR: ~a~%" (or (oget err "message") err))))
    (values-list nil) )
